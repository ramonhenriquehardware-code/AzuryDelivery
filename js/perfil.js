...
const AZURY_SACOLA_KEY = "azurySacola";
const AZURY_AVALIACOES_TABLE = "avaliacoes_pedido";

let azuryUsuarioPedidos = null;
let azuryEventosPedidosAtivos = false;
let azuryPedidoAvaliandoId = "";
let azuryNotaAvaliacao = 0;
...
function primeiroValorPedido(objeto, chaves, fallback = null) {
...
function obterUsuarioPedidosAtual() {
    ...
    if (azuryUsuarioPedidos) {
        return azuryUsuarioPedidos;
    }
...
async function salvarAvaliacaoPedido() {
    ...
    if (
        azuryNotaAvaliacao < 1 ||
        azuryNotaAvaliacao > 5
    ) {
        if (status) {
            status.textContent =
