document.addEventListener("DOMContentLoaded", async () => {
  ...
  function getAzuryBoxes() {
    const configured = state.config?.azury_boxes;
    ...
    return AZURY_BOX_DEFAULTS.map((fallback) => {
  ...
  function isAzuryBoxProduct(product) {
    ...
    return (
      String(product.produto_tipo || product.tipo || "").toLowerCase() ===
        "azury_box" || isAzuryBoxKey(product.produto_chave || product.chave)
    );
  }
  ...
  function productDisplayName(product) {
    if (product && typeof product === "object" && isAzuryBoxProduct(product)) {
      ...
      return String(
        product.produto_nome || product.nome || configured?.nome || "Azury Box",
      ).trim();
    }
  ...
  function renderBoxBuilderSizes(selectedKey = "") {
    ...
    container
      .querySelectorAll("input[name='tamanhoMonteSeuOpcao']")
      .forEach((input) => {
  ...
  function applyBuilderProductMode() {
    ...
    const counterTitle = document.querySelector(
      ".azury-complementos-progresso-titulo",
    );
    const regularHeader = d.middle?.querySelector(
      ".azury-complementos-secao:not(.especiais)",
    );
    const specialHeader = d.middle?.querySelector(
    ...
    if (sizeTitle)
      sizeTitle.textContent = boxMode
        ? "Escolha o tamanho da Box"
        : "Escolha o tamanho";
    if (structureTitle)
      structureTitle.textContent = boxMode
        ? "Complementos da Box"
        : "Estrutura do seu açaí";
    if (structureText) {
  ...
  function renderSizes() {
    const cupCard = document.querySelector("[data-azury-cups-card]");

    const cupOptions = document.getElementById("azuryCupOpcoes");

    const initialPrice = document.getElementById("azuryCupPrecoInicial");

    const cupButton = document.querySelector("[data-btn-azury-cups]");

    const visibleSizes = state.sizes.filter((item) => item.visivel !== false);

    const availableSizes = visibleSizes.filter(
      (item) => item.disponivel === true,
    );

    if (cupCard) {
      cupCard.hidden = visibleSizes.length === 0;
    }

    if (cupOptions) {
      cupOptions.innerHTML = visibleSizes
        .map((item) => {
    ...
    if (initialPrice) {
      initialPrice.textContent = firstAvailable
        ? `A partir de ${money(firstAvailable.preco_base)}`
        : "Indisponível no momento";
    }

    if (cupButton) {
      cupButton.dataset.tamanho = firstAvailable
        ? String(firstAvailable.tamanho_ml)
        : "";

      cupButton.dataset.precoBase = firstAvailable
        ? String(firstAvailable.preco_base)
        : "0";

      cupButton.dataset.disponibilidade = firstAvailable
        ? "disponivel"
        : "em-breve";

      cupButton.dataset.textoAberto = "Montar meu açaí";

      cupButton.disabled = !firstAvailable;
    }
    ...
    container.innerHTML = visibleSizes
      .map((item, index) => {
        const available = item.disponivel === true;

        const firstAvailableIndex = visibleSizes.findIndex(
          (size) => size.disponivel === true,
        );
        ...
        return `
                    ...
                    >
                        ...
                        <input
                            ...
                            ${available ? "" : "disabled"}
                            ${
                              index === firstAvailableIndex && available
                                ? "checked"
                                : ""
                            }
                        >
      ...
      input.addEventListener("change", () => {
        ...
        if (input.checked) {
          ...
          layerInputs.forEach((layerInput) => {
            layerInput.disabled = isAzuryBoxProduct(currentProductDescriptor());
          });
  ...
  function footerScheduleText() {
    ...
    const sameHours = active.every(
      (item) => item.open === active[0].open && item.close === active[0].close,
    );
    ...
    return active
      ...
      .map((item) => `${dayNames[item.day]} ${item.open}–${item.close}`)
      .join(" • ");
  }
  ...
  function updateFooterSchedule() {
    ...
    d.footerSchedule.textContent = footerScheduleText();
  }
  ...
  async function consultZip(zip) {
    ...
    try {
      ...
      state.zipCity = String(data.localidade || "").trim();

      state.zipState = String(data.uf || "").trim();
  ...
  function setupAddressCoordinateInvalidation() {
    [d.street, d.number, d.district].filter(Boolean).forEach((field) => {
  ...
  async function geocodeDeliveryAddress() {
    ...
    if (
      state.addressCoordinates?.signature === signature &&
      Number.isFinite(state.addressCoordinates.latitude) &&
      Number.isFinite(state.addressCoordinates.longitude)
    ) {
      return state.addressCoordinates;
    }

    const zip = d.zip?.value?.replace(/\D/g, "") || "";

    const street = d.street?.value?.trim() || "";

    const number = d.number?.value?.trim() || "";

    const district = d.district?.value?.trim() || "";

    const city = state.zipCity || "São Paulo";

    const uf = state.zipState || "SP";

    if (!zip || !street || !number || !district) {
      return null;
    }
    ...
    const queries = [
      ...
      {
        q: `${street}, ${number}, ${district}, ${city} - ${uf}, ${zip}, Brasil`,
      },
    ];
    ...
    for (const query of queries) {
      ...
      try {
        ...
        const latitude = normalizeCoordinate(results[0]?.lat, -90, 90);

        const longitude = normalizeCoordinate(results[0]?.lon, -180, 180);
        ...
        state.addressCoordinates = coordinates;

        return coordinates;
      } catch (error) {
        console.warn("Falha ao localizar as coordenadas do endereço.", error);
      }
    }
  ...
  async function createOrder() {
    ...
    message("Localizando o endereço da entrega...", "carregando");

    const addressCoordinates = await geocodeDeliveryAddress();
    ...
    if (!addressCoordinates) {
      ...
      d.send.textContent = "Enviar pedido";
    ...
    const payload = {
      ...
      itens: state.cart.map((item) => ({
        ...
        tamanho_label:
          item.tamanho_label ||
          (item.tamanho_ml ? `${item.tamanho_ml}ml` : null),
        tamanho_ml:
    ...
    try {
        ...
        .map((item, index) => {
          ...
          if (boxMode) {
            const complements = Array.from(
              new Set(
                (item.complementos || []).map((complement) => complement.nome),
              ),
            ).map((nome) => ({ nome }));
      ...
      const url = /Android|iPhone|iPad|iPod|IEMobile|Opera Mini/i.test(
        navigator.userAgent,
      )
  ...
  function bind() {
    $$(".btn-montar").forEach((button) => {
      button.addEventListener("click", async () => {
        ...
        if (
          button.dataset.produtoTipo === "azury_box" ||
          button.hasAttribute("data-btn-azury-box")
        ) {
          const preferredKey =
            button.dataset.produtoChave ||
            getAzuryBoxes().find(
              (box) => box.visivel !== false && box.disponivel !== false,
            )?.key ||
            "";
