(function () {
        ...
        ${items
          .map((item, index) => {
            ...
            const complementText = complements
              .map((c) =>
                firstDefined(c, ["nome", "complemento_nome"], "Complemento"),
              )
              .join(", ");
  ...
  function manualAzuryItemRowHtml(index) {
    ...
    const firstSize =
      sizes.find(
        (item) => item.disponivel === true && item.visivel !== false,
      ) ||
      sizes.find((item) => item.visivel !== false) ||
      {};

    const defaultSize = firstSize.tamanho_ml || "";

    const defaultPrice = firstSize.preco_base ?? "";
  ...
  function buildManualAzuryItem(row) {
    const size = Number(row.querySelector("[data-manual-size]")?.value);

    const quantity = Number(row.querySelector("[data-manual-quantity]")?.value);

    const unitPrice = parseMoneyInput(
      row.querySelector("[data-manual-price]")?.value,
    );

    if (!Number.isFinite(size) || size <= 0) {
      throw new Error("Escolha o tamanho de todos os itens.");
    }
    ...
    if (unitPrice < 0) {
      throw new Error("O valor de um item não pode ser negativo.");
    }

    const middle = splitManualComplements(
      row.querySelector("[data-manual-middle]")?.value,

      "meio",
    );

    const top = splitManualComplements(
      row.querySelector("[data-manual-top]")?.value,

      "cobertura",
    );

    return {
      produto_nome: `Monte o Seu • ${size}ml`,
  ...
  function printableOrderItemsHtml(order) {
    ...
    return items
      .map((item, index) => {
        const name = firstDefined(
          item,
          ["produto_nome", "nome", "produto", "nome_produto"],
          `Item ${index + 1}`,
        );
          ...
          layers.cobertura.length
            ? `
                  <p>
                    ...
                    ${escapeHtml(layers.cobertura.join(", "))}
                  </p>
                `
            : "",
      ...
      >
        ...
        <div class="data-card-head">
          ...
          <div>
            ...
            <p>
              ${escapeHtml(store.nome || "PH Sabor & Cia")}
            </p>
        ...
        <div class="data-pairs">
          ...
          <div class="data-pair">
            ...
            <strong>
              ${escapeHtml(store.whatsapp || "Não informado")}
            </strong>
          ...
          <div class="data-pair">
            ...
            <strong>
              ${escapeHtml(store.instagram || "Não informado")}
            </strong>
          ...
          <div class="data-pair">
            ...
            <strong>
              ${formatMoney(store.pedido_minimo || 0)}
            </strong>
          ...
          <div class="data-pair">
            ...
            <strong>
              ${escapeHtml(store.limite_entrega_km ?? "—")}
              km
            </strong>
